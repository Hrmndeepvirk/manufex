...
import {
  ...
  deleteSalesBonus,
  getSalesBonuses,
} from "@store/settings/employeeSetup/manageEmployee/salesBonusActions";
...
function Bonus({ employee }) {
  ...
  const appointmentPays = useSelector(
    (state) => state.settings.employeeSetup.salesBonus
  );
  ...
  useEffect(() => {
    if (employee?._id) {
      dispatch(getSalesBonuses(employee?._id, setLoading));
    }
  }, []);
  ...
  const onDelete = (id) => {
    dispatch(deleteSalesBonus(employee?._id, id));
  };
  ...
  return (
    <>
      <CustomDialog
        title={selectedBonus ? "Edit Bonus" : "Add Bonus"}
