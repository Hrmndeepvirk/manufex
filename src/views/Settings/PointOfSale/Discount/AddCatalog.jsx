...
const columns = [
  ...
  { field: "price", header: "Price", isCurrency: true },
  { field: "defaultQuantity", header: "Quantity" },
];

export default function AddCatalog({ data, onChange, name, label, filterFn }) {
  const dispatch = useDispatch();
  const allCatalogItems = useSelector(
    (state) => state.settings.inventory.catalogItems
  );
  const catalogItems = filterFn ? allCatalogItems?.filter(filterFn) : allCatalogItems;
